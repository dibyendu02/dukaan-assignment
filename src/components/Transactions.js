...
const TableCardItem=({orderId,refundDate,transactionfees,orderAmount})=>
{
    return (
        <tr className=' border-b'>
            <td className='text-[#146EB4] py-3.5 px-3 text-sm leading-5'>#{orderId}</td>
            <td className='text-text py-3.5 px-3 text-sm leading-5 text-center'>{refundDate}</td>
            <td className='text-text py-3.5 px-3 text-sm leading-5 text-right'>{transactionfees}</td>
            <td className='text-text py-3.5 px-3 text-sm leading-5 text-right'>{orderAmount}</td>
        </tr>
    )
}
...
const Transactions = () => {
  return (
    <div className="p-5">
      <h1 className="text-[#1A181E] text-xl font-medium">
      ...
      >
        ...
        <table className=" w-full text-left border-collapse ">
          <tr className="table-headers bg-[#F2F2F2] rounded-md">
            <th className="text-[#4D4D4D] font-medium py-1.5 px-3 border-[#F2F2F2] text-sm">
              Order ID
            </th>
            <th className="text-[#4D4D4D] font-medium py-1.5 px-3 border-[#F2F2F2] text-sm flex gap-2 justify-center items-center">
              Order date
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="8"
            ...
            <th className="text-[#4D4D4D] font-medium py-1.5 px-3 border-[#F2F2F2] text-sm text-right">
              Order amount
            </th>
            <th className="text-[#4D4D4D] font-medium py-1.5 px-3 border-[#F2F2F2] text-sm flex gap-2 justify-end items-end ">
                Transaction fees 
                <CiCircleInfo/>
            </th>
          ...
          {tableData.map(
            ({ orderId, refundDate, transactionfees, orderAmount }) => (
              <TableCardItem
                key={orderId}
                orderId={281209}
